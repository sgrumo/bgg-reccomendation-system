...
RUN apt-get update \
  && apt-get install -y --no-install-recommends libstdc++6 openssl libncurses6 locales ca-certificates curl postgresql-client \
  && rm -rf /var/lib/apt/lists/*
...
WORKDIR /app
RUN mkdir -p /backups && chown nobody /app /backups
